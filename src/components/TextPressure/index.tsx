...
const TextPressure: React.FC<TextPressureProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {(() => {
          // eslint-disable-next-line react-hooks/refs -- Resetting refs before collecting spans is intentional
          spansRef.current = [];

          const paragraphs = text.split("\n").map((p) => p.trim());

          return paragraphs.map((line, li) => (
            <div
              key={li}
              style={{
                display: "block",
            ...
            >
              {line.split(/\s+/).map((word, wi) => (
                ...
                >
                  ...
                  {wi < line.split(/\s+/).length - 1 && <span>&nbsp;</span>}
                </span>
              ))}

              {li < paragraphs.length - 1 && <br />}
            </div>
          ));
        })()}
      </h1>
